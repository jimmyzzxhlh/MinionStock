...
public class UpdateDividendJob extends DailyJob {
    ...
    public UpdateDividendJob() {
        ...
        type = new TypeToken<Map<String, Map<String, DividendData[]>>>(){}.getType();
    ...
    private void batchUpdateDividend(List<String> symbols) {
        ...
        Map<String, Map<String, DividendData[]>> dataMap = g.fromJson(str, type);        
        for (String symbol : symbols) {
            log.info("Checking the last updated dividend downloaded for " + symbol + " ...");
            DividendItem lastItem = DynamoDBHelper.getInstance().getLastItem(new DividendItem(symbol), DividendItem.class);
            List<DividendData> dataList = Arrays.asList(dataMap.get(symbol).get("dividends"));
            ...
            for (DividendData data : dataList) {
    ...
    private static Predicate<DividendData> filterInvalidDividend() {
        return data ->
